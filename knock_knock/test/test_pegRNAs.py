...
def test_intended_insertion_inference():
    for pegRNA_name, (start, end, strand) in [
        ...
        strat = knock_knock.editing_strategy.EditingStrategy(base_dir,
                                                             'pPC1000',
                                                             sgRNAs=pegRNA_name,
                                                            )
        assert len(strat.pegRNA_programmed_insertion_features) == 1
        inferred_insertion = strat.pegRNA_programmed_insertion_features[0]
...
def test_intended_deletion_inference():
    for pegRNA, expected_as_string in [
        ...
        expected = knock_knock.outcome.degenerate_indel_from_string(expected_as_string)
        assert (strat.pegRNA_programmed_deletion == expected)
...
def test_twin_prime_intended_deletion_inference():
    for pegRNA_pair, expected_as_string, is_prime_del in [
        ...
        for sequencing_start_feature_name in ['forward_primer', 'gDNA_reverse_primer']:
            ...
            expected = knock_knock.outcome.degenerate_indel_from_string(expected_as_string)
            assert (strat.pegRNA_programmed_deletion == expected)
            assert (strat.is_prime_del == is_prime_del)

def test_pegRNA_PBS_and_RTT_inference():
    strat = knock_knock.editing_strategy.EditingStrategy(base_dir, 'PAH_E4-2_45_EvoPreQ1-4_43_EvoPreQ1')

    feature = strat.features['PAH_E4', 'PAH_E4.2_45_EvoPreQ1_PBS']
    assert (feature.start, feature.end, feature.strand) == (612, 624, '-')

    feature = strat.features['PAH_E4', 'PAH_E4.4_43_EvoPreQ1_PBS'] 
    ...
    strat = knock_knock.editing_strategy.EditingStrategy(base_dir, 'EMX1', sgRNAs='EMX1_3b')

    target_PBS = strat.features['EMX1', 'EMX1_3b_PBS']
    assert (target_PBS.start, target_PBS.end, target_PBS.strand) == (653, 667, '+')
    pegRNA_PBS = strat.features['EMX1_3b', 'PBS']
    assert (pegRNA_PBS.start, pegRNA_PBS.end, pegRNA_PBS.strand) == (109, 123, '-')
    pegRNA_RTT = strat.features['EMX1_3b', 'RTT']
    assert (pegRNA_RTT.start, pegRNA_RTT.end, pegRNA_RTT.strand) == (96, 108, '-')

def test_twin_prime_overlap_inference():
    strat = knock_knock.editing_strategy.EditingStrategy(base_dir, 'HEK3_attB_A30_B30')

    A_feature = strat.features['HEK3_attB_A_30', 'overlap']
    B_feature = strat.features['HEK3_attB_B_30', 'overlap']
    ...
    feature_2_45 = strat.features['PAH_E4.2_45_EvoPreQ1', 'overlap']
    feature_4_43 = strat.features['PAH_E4.4_43_EvoPreQ1', 'overlap']
